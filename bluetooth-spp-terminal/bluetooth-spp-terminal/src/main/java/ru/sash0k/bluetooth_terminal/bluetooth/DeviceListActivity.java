...
public class DeviceListActivity extends Activity {
    ...
    private String long_click_address = "";
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        pairedListView.setOnItemLongClickListener(mDeviceClickListenerLong);
        ...
        if (pairedDevices != null && !pairedDevices.isEmpty()) {
            ...
            for (BluetoothDevice device : pairedDevices) {
                ...
                pairedDevicesAdapter.getPosition(device.getName() + '\n' + address);
            }
        } else {
    ...
    private final AdapterView.OnItemLongClickListener mDeviceClickListenerLong = new AdapterView.OnItemLongClickListener() {
        public boolean onItemLongClick(AdapterView<?> av, View v, int arg2, long arg3) {
            // Get the device MAC address, which is the last 17 chars in the View
            CharSequence info = ((TextView) v).getText();
            if (info != null) {
                long_click_address = info.toString().substring(info.length() - 17);
                AlertDialog.Builder builder = new AlertDialog.Builder(v.getContext());
                builder.setTitle(R.string.key_hint);
                final EditText input = new EditText(v.getContext());
                input.setInputType(InputType.TYPE_CLASS_TEXT | InputType.TYPE_TEXT_VARIATION_PASSWORD);
                builder.setView(input);
                builder.setPositiveButton(R.string.button_store, new DialogInterface.OnClickListener() {
                    @Override
